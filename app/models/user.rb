class User < ActiveRecord::Base
  ...
  def self.user_in_role?(user,role) #if bypass = true, admin user will not return true
   return user.user_in_role?(role)
  end


  def user_in_role?(role) #if bypass = true, admin user will not return true
    user_roles = self.roles.map{|x| x.role}
    return user_roles.include?(role.to_s) || user_roles.include?("admin") #admin bypass all
  end
